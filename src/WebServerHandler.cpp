...
String WebServerHandler::obtemMetricas() {
  ...
  int psram_size = 0;
  int free_psram_size = 0;
  int min_free_psram = 0;
  int max_alloc_psram = 0;
  ...
  String chipModel      = F("ESP8266");
  String chipRevision   = F("1");
  String chipRevisionDesc = chipModel + " Rev " + chipRevision;
  ...
  atribuiMetrica(&p, boardName+"_min_free_heap", String(ESP.getFreeHeap()));
  atribuiMetrica(&p, boardName+"_max_alloc_heap", String(ESP.getMaxFreeBlockSize()));
...
void WebServerHandler::handleInsertAsk() {
  server->on("/ask", HTTP_POST, [this](AsyncWebServerRequest * request){}, NULL,
    [this](AsyncWebServerRequest * request, uint8_t *data, size_t len, size_t index, size_t total) {
      ...
      /* TODO
       * Revisar o parâmetro precisa ser passado pelo json
       */
      String retorno = enviarMensagemParaChatGPT(token, mensagem);
...
String WebServerHandler::enviarMensagemParaChatGPT(String token, String mensagem) {
  String resposta = "";
  HTTPClient http;
